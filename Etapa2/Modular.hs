...
mapSingle :: (a -> a) -> [a] -> [[a]]
mapSingle f [] = []
mapSingle f xs = (f (head xs) : tail xs) 
                : (foldr (\x acc -> (head xs : x) : acc) [] (mapSingle f (tail xs)))
...
partitions :: [a] -> [[[a]]]
partitions [x] = [[[x]]] 
partitions xs = (map ([head xs] :) (partitions (tail xs))) ++ 
                (foldr (++) [] [mapSingle (head xs :) p | p <- partitions (tail xs)])
