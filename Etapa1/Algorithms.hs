...
search_aux :: (Ord a) => ([a] -> [a] -> [a]) -> Graph a -> S.Set a -> [a] -> [a]
search_aux f graph visited list = if null list then []
                                    else if (S.member (head list) visited) then search_aux f graph visited (tail list)
                                    else (head list) : search_aux f graph (S.insert (head list) visited) new_call_function
                                    where
                                        new_call_function = (f (tail list) (S.toList (outNeighbors (head list) graph)))
...
search f node graph = search_aux f graph S.empty [node]
...
countIntermediate from to graph = if elem to bfs_list 
                                    then Just (length (fst (span (< to) bfs_list)) - 1, length (fst (span (< to) dfs_list)) - 1)
                                    else Nothing
                                where
                                    bfs_list = bfs from graph
